...
TAROT_VERSION_PATTERN = re.compile(r"^(?P<base>.+?)(?P<version>\d+)$")
TAROT_DRAW_HISTORY = {}


def _parse_tarot_version(versioned_name: str):
    match = TAROT_VERSION_PATTERN.fullmatch(versioned_name)
    if not match:
        return None, None
    return match.group("base"), int(match.group("version"))


def _list_tarot_versions(card_name: str):
    versions = []
    for path in TAROT_IMAGE_DIR.glob(f"{card_name}*.png"):
        base_name, version = _parse_tarot_version(path.stem)
        if base_name == card_name and version is not None:
            versions.append((version, path.stem, path))
    versions.sort(key=lambda item: item[0])
    return versions


def _pick_tarot_image(card_name: str):
    versions = _list_tarot_versions(card_name)
    if not versions:
        return None, None

    version_names = [versioned_name for _, versioned_name, _ in versions]
    history = TAROT_DRAW_HISTORY.setdefault(card_name, [])
    history[:] = [item for item in history if item in version_names]

    available = [item for item in version_names if item not in history]
    if not available:
        history.clear()
        available = version_names.copy()

    selected_name = random.choice(available)
    history.append(selected_name)

    for _, versioned_name, path in versions:
        if versioned_name == selected_name:
            return selected_name, path
...
async def tool_draw_tarot_v2() -> str:
    """随机抽取一张塔罗牌，并尽量轮换同牌的不同图片版本。"""
    card = random.choice(TAROT_CARDS)
    position = random.choice(["正位", "逆位"])
    selected_version, card_image_path = _pick_tarot_image(card)

    image_path = None
    if card_image_path and card_image_path.exists():
        if position == "逆位":
            try:
                from PIL import Image as PILImage

                TEMP_IMAGE_DIR.mkdir(parents=True, exist_ok=True)
                reversed_name = selected_version or card
                reversed_path = TEMP_IMAGE_DIR / f"{reversed_name}_逆位.png"
                img = PILImage.open(card_image_path)
                img.rotate(180).save(reversed_path)
                image_path = str(reversed_path)
            except Exception as e:
    ...
    return (
        f"🎋 抽取结果: {card} ({position})\n"
        f"牌面版本: {selected_version or '暂无图片版本'}\n"
        f"日期: {current_date} {current_time_str}\n"
        "请根据这张塔罗牌为用户解读今日运势，保持可爱简短的语气，2-3句话即可，要结合牌面含义与用户的问题语境。"
    )


async def tool_list_tarot_cards_v2() -> str:
    """列出所有塔罗原始牌及其已上传的图片版本。"""
    lines = []
    total_versions = 0

    for card in TAROT_CARDS:
        versions = _list_tarot_versions(card)
        total_versions += len(versions)
        if versions:
            version_names = "、".join(versioned_name for _, versioned_name, _ in versions)
            lines.append(f"{card}: {version_names}")
        else:
            lines.append(f"{card}: 暂无图片版本")

    return f"共有 {len(TAROT_CARDS)} 张塔罗原始牌，当前有 {total_versions} 个图片版本:\n" + "\n".join(lines)
...
TOOL_MAP = {
    ...
    "draw_tarot": tool_draw_tarot_v2,
    "list_tarot_cards": tool_list_tarot_cards_v2,
