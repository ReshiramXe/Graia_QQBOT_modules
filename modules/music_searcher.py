...
# API密钥从YAML配置加载（环境变量可覆盖）
api_key = os.environ.get("KINOKO_API_KEY") or config["api"]["kinoko"]["api_key"]


def search_music(keyword):
    url = f'http://bgm.pokemonhome.cn/search/ajax_search_music?text={keyword}'
    response = requests.get(url)
    response.encoding = 'utf-8'
    data = response.json()
    music_id = [item['id'] for item in data][0]

    url2 = f'http://bgm.pokemonhome.cn/open/get_music_info?api_key={api_key}&music_id={music_id}'
...
def get_random_music():
    try:
        url = f'http://bgm.pokemonhome.cn/open/get_random_music_id?api_key={api_key}&sound=false&user_submission=true'
        response = requests.get(url)
        response.encoding = 'utf-8'
        data = response.json()
