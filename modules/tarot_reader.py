...
def compress_image(image_path: Path, max_size: int = 300) -> str:
    """压缩图片到指定最大尺寸，保存到临时目录并返回路径"""
        ...
        try:
            ...
            for card in tarot_cards:
                ...
                if card in config.get("card_authors", {}):
                    ...
                    author_info = f"\n添加者: {author.get('user_name', '未知')} (ID: {author.get('user_id', '未知')})"

                message_chain = [Plain(f"🎴 {card}{author_info}")]
                card_image = get_card_image_path(card)
                if card_image.exists():
                    compressed_path = compress_image(card_image)
                    message_chain.append(GraiaImage(path=compressed_path))
