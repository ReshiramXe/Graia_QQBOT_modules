...
> Agent 模式开启后，`@Bot` 的对话将支持 tool-calling，LLM 可自主调用文件系统和数据库来回答问题。

| 指令 | 说明 | 模块 |
|------|------|------|
| `yb切换模式` | 开启 Agent 模式（仅管理员） | `agent_mode.py` |
| `yb结束模式` | 关闭 Agent 模式，恢复普通聊天（仅管理员） | `agent_mode.py` |
...
| 📄 读取文件 | 读取项目内的文本文件内容 |
| 📁 浏览目录 | 列出指定目录下的文件结构 |
| 🔍 搜索文件 | 按 glob 模式搜索文件（如 `*.py`） |
| 🗄️ 数据库查询 | 执行 SELECT 查询（仅只读） |
| 💬 群聊记录 | 读取本群最近的聊天消息 |
| 🔎 聊天搜索 | 按关键词搜索群聊历史 |
| 🏷️ 用户印象 | 查询指定群友的印象记录 |

**配置：** 复制 `config/agent_mode.example.yaml` 为 `agent_mode.yaml`，可自定义触发指令、回复文案、Agent 人格、安全限制等。
